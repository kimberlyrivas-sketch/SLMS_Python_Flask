...
def init_db():
    ...
    try:
        cursor.execute("ALTER TABLE books ADD COLUMN cover_url MEDIUMTEXT")
    except Exception:
        pass
...
def api_add_book():
    data = request.json
    new_id = query(
        "INSERT INTO books (title, author, category, price, copies, cover_url) VALUES (%s,%s,%s,%s,%s,%s)",
        (data["title"], data["author"], data.get("category", "Other"),
         float(data.get("price", 0)), int(data.get("copies", 1)),
         data.get("cover_url", None)), commit=True)
...
def api_update_book(book_id):
    data = request.json
    query("UPDATE books SET title=%s, author=%s, category=%s, price=%s, copies=%s, cover_url=%s WHERE id=%s",
          (data["title"], data["author"], data["category"],
           float(data["price"]), int(data["copies"]),
           data.get("cover_url", None), book_id), commit=True)
...
@app.route("/api/reservations/<int:res_id>/approve", methods=["POST"])
@login_required
def api_approve_reservation(res_id):
    """Convert a reservation into an active borrow request (approved)."""
    res = query("SELECT * FROM reservations WHERE id=%s", (res_id,), one=True)
    if not res:
        return jsonify({"error": "Not found"}), 404
    today = datetime.now().strftime("%Y-%m-%d")
    due   = (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d")
    # Mark reservation as approved
    query("UPDATE reservations SET status='approved' WHERE id=%s", (res_id,), commit=True)
    # Create a borrow record directly
    query("""INSERT INTO borrows (userId, bookId, status, borrowedAt, dueDate, penalty)
             VALUES (%s,%s,'borrowed',%s,%s,0)""",
          (res["userId"], res["bookId"], today, due), commit=True)
    query("UPDATE books SET copies = GREATEST(0, copies - 1) WHERE id=%s", (res["bookId"],), commit=True)
    return jsonify({"ok": True})

@app.route("/api/reservations/<int:res_id>/decline", methods=["POST"])
@login_required
def api_decline_reservation(res_id):
    """Cancel / decline a pending reservation."""
    res = query("SELECT * FROM reservations WHERE id=%s", (res_id,), one=True)
    if not res:
        return jsonify({"error": "Not found"}), 404
...
def get_available_gemini_models():
    """Dynamically fetch available models from the API."""
    try:
        resp = req_lib.get(
        ...
        if "models" not in data:
            return GEMINI_MODELS  # fallback to hardcoded list
        available = []
        for m in data.get("models", []):
            name = m.get("name", "").replace("models/", "")
            methods = m.get("supportedGenerationMethods", [])
            if "generateContent" in methods and (
                "flash" in name or "pro" in name
            ) and "vision" not in name and "embedding" not in name:
                available.append(name)
        # prefer 2.x models first
        preferred = [n for n in available if n.startswith("gemini-2")]
        others    = [n for n in available if not n.startswith("gemini-2")]
        return (preferred + others) or GEMINI_MODELS
    except Exception:
...
def api_chat():
    ...
    my_borrows     = query(
        "SELECT br.*, bk.title FROM borrows br JOIN books bk ON bk.id=br.bookId "
        "WHERE br.userId=%s AND br.status='borrowed'", (user["id"],))
    my_reqs        = query(
        "SELECT rq.*, bk.title FROM borrow_requests rq JOIN books bk ON bk.id=rq.bookId "
        "WHERE rq.studentId=%s AND rq.status='pending'", (user["id"],))
    total_borrowed = query("SELECT COUNT(*) as c FROM borrows WHERE status='borrowed'", one=True)["c"]
    ...
    book_list      = "\n".join(
        f'- "{b["title"]}" by {b["author"]} ({b["category"]}) — {b["copies"]} copies available'
...
Guidelines: Be friendly, concise (under 150 words), and help with borrows or finding books."""

    # Build Gemini-compatible message history (must alternate user/model)
    raw = messages[-10:]
    gemini_messages = []
    last_role = None
    for m in raw:
        ...
        for attempt in range(2):
            try:
                ...
                if error_code == 404 or "not found" in error_msg.lower():
                    print(f"[chat] Model {model} not found, trying next...")
                    break

                # Rate limited → brief wait then retry same model once
                if error_code in [429, 503] or "quota" in error_msg.lower() or "high demand" in error_msg.lower():
                    if attempt == 0:
                        time.sleep(1.5)
                        continue
                    else:
                ...
                # Other API error
                print(f"[chat] API error from {model}: {error_msg}")
                break

            except req_lib.exceptions.Timeout:
                print(f"[chat] Timeout on {model}")
                break
            except Exception as e:
                print(f"[chat] Exception on {model}: {e}")
                break
